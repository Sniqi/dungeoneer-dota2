...
function customSchema:init()
    ...
    -- Flag Example
    statCollection:setFlags({ version = DUNGEONEER_VERSION })
        ...
        if PlayerResource:IsValidPlayerID(playerID) then
            if not PlayerResource:IsBroadcaster(playerID) then
                ...
                table.insert(players, {
                    ...
                    HeroLevel = hero:GetLevel(), -- Level
                    HeroNetworth = GetNetworth(PlayerResource:GetSelectedHeroEntity(playerID)), -- Networth
                    HeroDeaths = hero:GetDeaths(), -- Deaths

                    --Items:
                    HeroItem1 = GetItemSlot(hero, 0),
                    HeroItem2 = GetItemSlot(hero, 1),
                    HeroItem3 = GetItemSlot(hero, 2),
                    HeroItem4 = GetItemSlot(hero, 3),
                    HeroItem5 = GetItemSlot(hero, 4),
                    HeroItem6 = GetItemSlot(hero, 5)
                })
            end
        end
    end
