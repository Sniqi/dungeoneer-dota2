lesser_main_attribute = class({})

function lesser_main_attribute:OnCreated( data )
	-- ### VALUES START ### --
	self.main_attribute                = 12
	self.attributes_max                = 48
	-- ### VALUES END ### --
...
function lesser_main_attribute:OnIntervalThink()
	if not IsServer() then return end
	local caster = self:GetParent()
...
function lesser_main_attribute:DeclareFunctions()
	local funcs = {
...
function lesser_main_attribute:GetModifierBonusStats_Strength( params )
	return self:GetStackCount() * self.str
end

function lesser_main_attribute:GetModifierBonusStats_Agility( params )
	return self:GetStackCount() * self.agi
end

function lesser_main_attribute:GetModifierBonusStats_Intellect( params )
	return self:GetStackCount() * self.int
end

function lesser_main_attribute:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE + MODIFIER_ATTRIBUTE_MULTIPLE
end

function lesser_main_attribute:IsHidden()
    return true
end

function lesser_main_attribute:IsPurgable()
	return false
end

function lesser_main_attribute:IsPurgeException()
	return false
end

function lesser_main_attribute:IsStunDebuff()
	return false
end

function lesser_main_attribute:IsDebuff()
	return false
end
