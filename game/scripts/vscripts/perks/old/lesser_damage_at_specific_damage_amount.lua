lesser_damage_at_specific_damage_amount = class({})

function lesser_damage_at_specific_damage_amount:OnCreated( data )
	-- ### VALUES START ### --
	self.boss_hp                       = 4
	self.all_attributes                = 100
...
function lesser_damage_at_specific_damage_amount:OnRemoved()
	if not IsServer() then return end
	Change_PerkEffectivenessStats(self, false)
end

function lesser_damage_at_specific_damage_amount:DeclareFunctions()
	local funcs = {
...
function lesser_damage_at_specific_damage_amount:OnTakeDamage( params )
	if not IsServer() then return end
...
function lesser_damage_at_specific_damage_amount:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE + MODIFIER_ATTRIBUTE_MULTIPLE
end

function lesser_damage_at_specific_damage_amount:IsHidden()
    return true
end

function lesser_damage_at_specific_damage_amount:IsPurgable()
	return false
end

function lesser_damage_at_specific_damage_amount:IsPurgeException()
	return false
end

function lesser_damage_at_specific_damage_amount:IsStunDebuff()
	return false
end

function lesser_damage_at_specific_damage_amount:IsDebuff()
	return false
end
