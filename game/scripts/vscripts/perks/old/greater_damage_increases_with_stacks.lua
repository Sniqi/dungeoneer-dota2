greater_damage_increases_with_stacks = class({})

function greater_damage_increases_with_stacks:OnCreated( data )
	-- ### VALUES START ### --
	self.all_attributes                = 20
	self.bonus                         = 20
	self.stacks_max                    = 5
	...
	self.id = DoUniqueString("greater_damage_increases_with_stacks")
end

function greater_damage_increases_with_stacks:OnRemoved()
	if not IsServer() then return end
	Change_PerkEffectivenessStats(self, false)
end

function greater_damage_increases_with_stacks:DeclareFunctions()
	local funcs = {
...
function greater_damage_increases_with_stacks:OnTakeDamage( params )
	if not IsServer() then return end
...
function greater_damage_increases_with_stacks:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE + MODIFIER_ATTRIBUTE_MULTIPLE
end

function greater_damage_increases_with_stacks:IsHidden()
    return true
end

function greater_damage_increases_with_stacks:IsPurgable()
	return false
end

function greater_damage_increases_with_stacks:IsPurgeException()
	return false
end

function greater_damage_increases_with_stacks:IsStunDebuff()
	return false
end

function greater_damage_increases_with_stacks:IsDebuff()
	return false
end
