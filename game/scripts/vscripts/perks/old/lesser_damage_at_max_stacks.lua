lesser_damage_at_max_stacks = class({})

function lesser_damage_at_max_stacks:OnCreated( data )
	-- ### VALUES START ### --
	self.stack_trigger                 = 4
	self.all_attributes                = 75
...
function lesser_damage_at_max_stacks:OnRemoved()
	if not IsServer() then return end
	Change_PerkEffectivenessStats(self, false)
end

function lesser_damage_at_max_stacks:DeclareFunctions()
	local funcs = {
...
function lesser_damage_at_max_stacks:OnTakeDamage( params )
	if not IsServer() then return end
...
function lesser_damage_at_max_stacks:OnProjectileHit( hTarget, vLocation )

end

function lesser_damage_at_max_stacks:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE + MODIFIER_ATTRIBUTE_MULTIPLE
end

function lesser_damage_at_max_stacks:IsHidden()
    return true
end

function lesser_damage_at_max_stacks:IsPurgable()
	return false
end

function lesser_damage_at_max_stacks:IsPurgeException()
	return false
end

function lesser_damage_at_max_stacks:IsStunDebuff()
	return false
end

function lesser_damage_at_max_stacks:IsDebuff()
	return false
end
