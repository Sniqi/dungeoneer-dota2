greater_damage_main_attribute_at_max_stacks = class({})

function greater_damage_main_attribute_at_max_stacks:OnCreated( data )
	-- ### VALUES START ### --
	self.stack_trigger                 = 8
	self.main_attribute                = 60
	self.duration                      = 10
	self.cooldown                      = 10
...
function greater_damage_main_attribute_at_max_stacks:OnRemoved()
	if not IsServer() then return end
	Change_PerkEffectivenessStats(self, false)
end

function greater_damage_main_attribute_at_max_stacks:DeclareFunctions()
	local funcs = {
...
function greater_damage_main_attribute_at_max_stacks:OnTakeDamage( params )
	if not IsServer() then return end
...
function greater_damage_main_attribute_at_max_stacks:GetModifierBonusStats_Strength( params )
	local caster = self:GetParent()
...
function greater_damage_main_attribute_at_max_stacks:GetModifierBonusStats_Agility( params )
	local caster = self:GetParent()
...
function greater_damage_main_attribute_at_max_stacks:GetModifierBonusStats_Intellect( params )
	local caster = self:GetParent()
...
function greater_damage_main_attribute_at_max_stacks:GetModifierPhysicalArmorBonus( params )

end

function greater_damage_main_attribute_at_max_stacks:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE + MODIFIER_ATTRIBUTE_MULTIPLE
end

function greater_damage_main_attribute_at_max_stacks:IsHidden()
    return true
end

function greater_damage_main_attribute_at_max_stacks:IsPurgable()
	return false
end

function greater_damage_main_attribute_at_max_stacks:IsPurgeException()
	return false
end

function greater_damage_main_attribute_at_max_stacks:IsStunDebuff()
	return false
end

function greater_damage_main_attribute_at_max_stacks:IsDebuff()
	return false
end
